...
class WikipediaCollector {
  ...
  cleanSentence(sentence) {
    // Strip Wiki/HTML markup and formatting
    ...
    const lemmas = words.map(word => {
      const lemmaList = lemmatizer.lemmas(word);
      return lemmaList.length > 0 && lemmaList[0].length > 0 ? lemmaList[0][0] : word;
    });

    if (lemmas.length < 4) return null;
    return lemmas.join(' ');
  }
  ...
  async collectFromDump(dumpPath, targetSentenceCount = 100000) {
    ...
    const decompressed = stream.pipe(bz2());
